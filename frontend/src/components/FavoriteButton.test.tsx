...
describe('FavoriteButton', () => {
  ...
  it('無効化されている場合、クリックできない', async () => {
    const user = userEvent.setup();
    const onToggle = vi.fn();
    render(<FavoriteButton isFavorited={false} onToggle={onToggle} disabled={true} />);
  ...
  it('異なるサイズを適用できる', () => {
    const onToggle = vi.fn();
    const { rerender } = render(<FavoriteButton isFavorited={false} onToggle={onToggle} size="sm" />);

    let button = screen.getByRole('button');
    expect(button).toHaveClass('w-6', 'h-6');

    rerender(<FavoriteButton isFavorited={false} onToggle={onToggle} size="md" />);
    button = screen.getByRole('button');
    expect(button).toHaveClass('w-8', 'h-8');

    rerender(<FavoriteButton isFavorited={false} onToggle={onToggle} size="lg" />);
    button = screen.getByRole('button');
