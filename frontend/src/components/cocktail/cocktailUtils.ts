...
export const buildDefaultDescription = (cocktail: Cocktail | null) => {
  ...
  const baseStory =
    BASE_STORIES[cocktail.base] ?? '素材の持つ香りを素直に引き出し、口いっぱいに優雅な余韻が広がります。';
  const strengthStory =
    STRENGTH_STORIES[cocktail.strength] ?? '飲み進めるほどに調和が増し、余白のある味わいになります。';
  const techniqueStory =
    TECHNIQUE_STORIES[cocktail.technique] ?? '丁寧な仕上げによって、香りと温度が最適なバランスに整います。';
  return [
    baseStory,
    strengthStory,
    techniqueStory,
